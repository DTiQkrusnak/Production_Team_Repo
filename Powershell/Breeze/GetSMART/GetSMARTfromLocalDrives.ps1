function Invoke-Breezev2 {
    ...
    function Get-SMARTInfo {
        ...
        return $disks  #| Out-Null
        
    }

    function GetPhysicalDiskInfo ($disks) {
        $unhealhyDrivesList = New-Object System.Collections.Generic.List[PSCustomObject]

        foreach ($drive in $disks) {
            if ($drive.Health -eq "Healthy") {
                $object = [PSCustomObject]@{
                    Model     = $drive.Caption
                    MediaType = $drive.MediaType
                    Health    = $drive.Health
                }
                $unhealhyDrivesList.Add($object)
            }
        }
        return $unhealhyDrivesList
    }

    function ListFaultySmart ($getSmart) {
        $driveLetters = Get-PhysicalDisk | ForEach-Object {
            $physicalDisk = $_
            $physicalDisk |
            Get-Disk |
            Get-Partition |
            Where-Object DriveLetter |
            Select-Object DriveLetter, @{n = 'Name'; e = { $physicalDisk.FriendlyName } }
        }

        foreach ($selectedDrive in $GetSmart) {
            $isOK = $false
            $objectsFaulty = @{}
            $script:selectedDrive | Out-Null
            $faultyObjects = $selectedDrive.SMART | Where-Object { ($_.IsOK -eq $isOK) } | Select-Object Name, isok
            #$faultyObjects | ConvertTo-Json
    
            if ($null -ne $faultyObjects) {
                $objectsFaulty = [PSCustomObject]@{
                    Name        = $selectedDrive.Model
                    DriveLetter = $driveLetters | Where-Object { $_.Name -eq $selectedDrive.Model } | Select-Object DriveLetter
                    Smart       = $faultyObjects
                }       
            }
            return $objectsFaulty
        }
    }



    #get general Healhy, unhealthy status (Get-physicalDisk)
    function GetPhysicalDiskInfo ($disks) {
        $unhealhyDrivesList = New-Object System.Collections.Generic.List[PSCustomObject]
    ...
    function sendRestData {
        $body = @{
            ...
            result          = GetPhysicalDiskInfo $getSmartVariable | ConvertTo-Json
            optionalResult1 = ListFaultySmart $getSmartVariable | ConvertTo-Json
            optionalResult2 = $getSmartVariable | ConvertTo-Json -Depth 4
            errorCode       = "NULL"
            errorDetails    = "NULL"
    ...
    CheckDatabaseState
    $getSmartVariable = Get-SMARTInfo
    getIdToken
    sendRestData
    <#
    Write-Host "------------- result: -------------" -ForegroundColor Green
    GetPhysicalDiskInfo $getSmartVariable | ConvertTo-Json
    Write-Host "--------  optionalresult1: --------" -ForegroundColor Red
    ListFaultySmart $getSmartVariable | ConvertTo-Json
    Write-Host "--------  optionalresult2: --------" -ForegroundColor Yellow
    $getSmartVariable | ConvertTo-Json -Depth 4
    #>
}
